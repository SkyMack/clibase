...
func addLogFlags(flags *pflag.FlagSet) {
	...
	SetFlagsFromEnvWithOverrides(cliBaseFlagPrefix, logFlags, map[string]string{
		logFlagFormatName: EnvNameForFlag("", logFlags.Lookup(logFlagFormatName)),
		logFlagLevelName:  EnvNameForFlag("", logFlags.Lookup(logFlagLevelName)),
	})
